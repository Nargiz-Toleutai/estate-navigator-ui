...
export default function ThemeColorPicker() {
  ...
  return (
    <details
      className={[styles.themeColorPicker, dirty && "dirty"]
        .filter(Boolean)
        .join(" ")}
      ref={detailsRef}
    >
      ...
      <div>
        {cssColorVars.map((colorVar) => (
          <PickerColorInput key={colorVar} cssColorVar={colorVar} />
        ))}
      </div>
    </details>
  );
      ...
      toast
        .promise(
          navigator.clipboard.writeText(color),
          {
            ...
            success: (
              <span>
                Цвет <b style={{ color }}>{color}</b> скопирован в буфер обмена
              </span>
            ),
            error: (
              <span>
                Цвет <b>{color}</b> не был скопирова в буфер обмена :(
              </span>
            ),
          },
          {
